...
import { updateClass, uploadClassImage, deleteClassImage } from "../../lib/teacherFirebase";
...
const EditClassModal = ({ isOpen, onClose, onClassUpdated, classData }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen && classData) {
      setFormData({
        ...
        hour: classData.hour || 1,
        community_partner: classData.community_partner || "",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!user || !classData) {
      toast.error("An error occurred. Missing user or class data.");
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      let newImageUrl = classData.imageUrl;

      if (imageFile) {
        newImageUrl = await uploadClassImage(classData.id, imageFile);
      } else if (imagePreview === null && classData.imageUrl) {
        await deleteClassImage(classData.id, classData.imageUrl);
        newImageUrl = "";
      }
      updatedData.imageUrl = newImageUrl;

      await updateClass(user.uid, classData.id, updatedData);
      ...
      toast.error(error.message || "Failed to update class", { id: toastId });
    } finally {
      setLoading(false);
    }
  };
                ...
                <label key={day} className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        setFormData(prev => ({ ...prev, availableDays: [...days, day] }));
                      } else {
                        setFormData(prev => ({ ...prev, availableDays: days.filter(d => d !== day) }));
                      }
                    }}
                  />
                  <span>{day}</span>
